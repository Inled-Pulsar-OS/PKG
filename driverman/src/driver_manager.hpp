...
// Package manager / distribution platform driverman runs on. Every command
// (install/remove/query/initramfs) is dispatched through this so the same
// driver logic works on Arch Linux (pacman) and Debian/Pulsar (apt/dpkg).
enum class Platform {
    UNKNOWN,
    ARCH,     // pacman + optionally an AUR helper (paru/yay/trizen)
    DEBIAN    // apt-get + dpkg
};
...
struct DriverInfo {
    ...
    bool        installed;
    bool        recommended;
    bool        fromAUR;     // requires AUR helper (yay/paru) — Arch only
    bool        openSource;  // true = no proprietary blobs
    bool        available    = true;   // false = no package exists for this platform (e.g. AUR legacy branches on Debian)
...
private:
    ...
    // (Arch: asked via pacman; Debian: standard linux-image-$(uname -r) name).
    ...
    std::string detectRunningKernelPackage();
    // Kernel headers package for the running kernel ("linux" -> "linux-headers"
    // on Arch, "linux-image-X-amd64" -> "linux-headers-X-amd64" on Debian).
    ...
    bool runInstall(const std::vector<std::string>& packages);
    bool runRemove(const std::vector<std::string>& packages);

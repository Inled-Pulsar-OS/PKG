...
// ── platform abstraction (Arch/pacman vs Debian/apt) ─────────────────────────
//
// driverman runs on both Pulsar OS editions. The Pulsar branding keeps
// ID=pulsaros in /etc/os-release but sets ID_LIKE=debian (Debian build) or
// ID_LIKE=arch (Arch build), which is what we key on — with the presence of
// the package manager binaries as a fallback.
...
Platform detectPlatform() {
    std::string rel;
    if (FILE* f = std::fopen("/etc/os-release", "r")) {
        std::array<char, 1024> buf;
        size_t n;
        while ((n = std::fread(buf.data(), 1, buf.size() - 1, f)) > 0) {
            buf[n] = '\0';
            rel += buf.data();
        }
        std::fclose(f);
    ...
    if (lower.find("id_like=arch") != std::string::npos ||
        lower.find("id=arch") != std::string::npos)
        return Platform::ARCH;
    if (lower.find("id_like=debian") != std::string::npos ||
        lower.find("id=debian") != std::string::npos ||
        lower.find("id=ubuntu") != std::string::npos)
        return Platform::DEBIAN;
    if (!execCmd("which pacman 2>/dev/null").empty()) return Platform::ARCH;
    if (!execCmd("which apt-get 2>/dev/null").empty()) return Platform::DEBIAN;
    return Platform::UNKNOWN;
}

static const char* platformLabel(Platform p) {
    switch (p) {
        case Platform::ARCH:   return "Arch Linux (pacman)";
        case Platform::DEBIAN: return "Debian/Pulsar (apt)";
        default:               return "Unknown platform";
    }
}

static std::string sudoCmd(const std::string& c) {
    return (geteuid() == 0) ? c : ("sudo " + c);
}

// Arch package name → the actual installable package on this platform.
// On Arch the name is identical; on Debian it maps to the apt equivalent.
// Empty result = no Debian equivalent exists (AUR-only legacy branches,
// Arch-specific kernel variants, lib32/multiarch-only packages, ...).
static std::string platformPkgName(const std::string& pkg) {
    if (detectPlatform() != Platform::DEBIAN) return pkg;

    struct Map { const char* arch; const char* deb; };
    static const std::vector<Map> table = {
        ...
        {"xf86-video-vesa",          "xserver-xorg-video-vesa"},
        {"xf86-video-fbdev",         "xserver-xorg-video-fbdev"},
        {"xf86-video-vmware",        "xserver-xorg-video-vmware"},
        {"xf86-video-qxl",           "xserver-xorg-video-qxl"},
        {"xf86-video-openchrome",    "xserver-xorg-video-openchrome"},
        {"xf86-video-savage",        "xserver-xorg-video-savage"},
        {"xf86-video-s3virge",       "xserver-xorg-video-s3virge"},
        {"vulkan-radeon",            "mesa-vulkan-drivers"},
        {"vulkan-intel",             "mesa-vulkan-drivers"},
        {"intel-compute-runtime",    "intel-opencl-icd"},
        {"intel-media-driver",       "intel-media-driver"},
        {"level-zero-loader",        "intel-level-zero-gpu"},
        {"intel-gpu-tools",          "intel-gpu-tools"},
        {"libva-intel-driver",       "libva-intel-driver"},
        {"rocm-opencl-runtime",      ""},
        {"nvidia",                   "nvidia-driver"},
        {"nvidia-utils",             "libnvidia-gl-1"},
        {"nvidia-dkms",              "nvidia-kernel-dkms"},
        {"nvidia-open",              ""},
        {"nvidia-open-dkms",         "nvidia-open-kernel-dkms"},
        {"nvidia-580xx-dkms",        ""},
        {"nvidia-580xx-utils",       ""},
        {"nvidia-470xx-dkms",        ""},
        {"nvidia-470xx-utils",       ""},
        ...
        {"gtkmm3",                   "libgtkmm-3.0-1t64"},
        {"virtualbox-guest-utils",   "virtualbox-guest-utils"},
        {"virtualbox-guest-modules-arch", "virtualbox-guest-dkms"},
    ...
    for (const auto& m : table)
        if (pkg == m.arch) return m.deb;
    return pkg;  // not a known Arch name: pass through (already platform name)
}

// Human hint for removing a package, adjusted to the running platform.
static std::string removeHintCmd() {
    return detectPlatform() == Platform::DEBIAN ? "sudo apt-get purge" : "sudo pacman -R";
}
...
std::string DriverManager::detectRunningKernelPackage() {
    ...
    if (detectPlatform() == Platform::DEBIAN) {
        // Standard Debian/Pulsar naming: linux-image-$(uname -r).
        // e.g. "6.1.0-18-amd64" -> "linux-image-6.1.0-18-amd64".
        return "linux-image-" + kernelRelease;
    }
...
bool DriverManager::isInstalled(const std::string& pkg) {
    std::string name = platformPkgName(pkg);
    if (name.empty()) return false;

    if (detectPlatform() == Platform::DEBIAN) {
        // dpkg-query -W -f='${db:Status-Abbrev}' prints "ii" for an installed
        // package and nothing for an unknown one (exit != 0).
        std::string out = execCmd("dpkg-query -W -f='${db:Status-Abbrev}' " + name + " 2>/dev/null");
    ...
    std::string out = execCmd("pacman -Q " + name + " 2>/dev/null");
    std::istringstream ss(out);
    std::string line;
    while (std::getline(ss, line)) {
        std::istringstream ls(line);
        std::string n, version;
        ls >> n >> version;
        if (n == name && !version.empty()) return true;
    }
    return false;
}
...
bool DriverManager::isAvailable(const std::string& pkg) {
    ...
    if (detectPlatform() == Platform::DEBIAN) {
        // `apt-cache show` prints a stanza for known packages and nothing for
        // unknown ones — much simpler to parse than `apt-cache policy`.
        std::string out = execCmd("apt-cache show " + name + " 2>/dev/null");
        return !out.empty();
    }
    ...
    std::string out = execCmd("pacman -Ss '^" + name + "$' 2>/dev/null");
    std::istringstream ss(out);
    std::string line;
    while (std::getline(ss, line)) {
...
bool DriverManager::runInstall(const std::vector<std::string>& packages) {
    // Translate to platform package names (dedup, skip AUR-only on Debian).
    std::vector<std::string> names;
    for (const auto& p : packages) {
        std::string n = platformPkgName(p);
        if (n.empty()) {
            std::cout << C_YELLOW << "  ⏭ Skipping '" << p
                      << "' — no package exists for this platform.\n" << C_RESET;
            continue;
        }
    ...
    if (detectPlatform() == Platform::DEBIAN) {
        if (noConfirm_) {
            // Unattended: noninteractive so apt never blocks on a prompt.
            cmd = "DEBIAN_FRONTEND=noninteractive " +
                  sudoCmd("apt-get install -y --no-install-recommends");
        } else {
            // Interactive: let apt run and prompt so the user keeps control
            // and can decide how to resolve any package conflict.
            cmd = sudoCmd("apt-get install --no-install-recommends");
        }
    } else {
        cmd = sudoCmd("pacman -S --needed");
        if (noConfirm_) cmd += " --noconfirm";
    }
    for (const auto& n : names) cmd += " " + n;
    std::cout << C_CYAN << "[driverman] Running: " << cmd << C_RESET << "\n";
    return system(cmd.c_str()) == 0;
}

bool DriverManager::runRemove(const std::vector<std::string>& packages) {
    std::vector<std::string> names;
    for (const auto& p : packages) {
        std::string n = platformPkgName(p);
        if (n.empty()) continue;
    ...
    if (detectPlatform() == Platform::DEBIAN) {
        cmd = noConfirm_
            ? "DEBIAN_FRONTEND=noninteractive " + sudoCmd("apt-get purge -y")
            : sudoCmd("apt-get purge");
    } else {
        cmd = sudoCmd("pacman -R");
        if (noConfirm_) cmd += " --noconfirm";
    }
    for (const auto& n : names) cmd += " " + n;
    std::cout << C_CYAN << "[driverman] Running: " << cmd << C_RESET << "\n";
    ...
    // First attempt failed (e.g. "target not found" or a held package).
    // Re-check each package against the local db with the same strict match
    // isInstalled() uses — if it's genuinely already gone, that's success,
    // not failure. Anything still genuinely installed gets one retry with
    // dependency/conflict checks relaxed (apt --allow-change-held-packages,
    // pacman -Rdd), which resolves some local-db bookkeeping edge cases
    // plain -R/-purge rejects.
    ...
    std::string retryCmd;
    if (detectPlatform() == Platform::DEBIAN)
        retryCmd = "DEBIAN_FRONTEND=noninteractive " +
                   sudoCmd("apt-get purge -y --allow-change-held-packages");
    else {
        retryCmd = sudoCmd("pacman -Rdd");
        if (noConfirm_) retryCmd += " --noconfirm";
    }
    for (const auto& n : stillInstalled) retryCmd += " " + n;
    std::cout << C_YELLOW << "[driverman] Retrying: " << retryCmd << C_RESET << "\n";
...
bool DriverManager::runAUR(const std::vector<std::string>& packages) {
    ...
    // AUR is Arch-only. On Debian the recommended driver list never contains
    // AUR packages (they map to nothing / are flagged unavailable), so this
    // only triggers if something slipped through — tell the user instead of
    // trying to run a helper that does not belong to this platform.
    if (detectPlatform() == Platform::DEBIAN) {
        std::cerr << C_RED
                  << "[driverman] AUR packages are Arch-only.\n"
                     "  On Debian/Pulsar this driver has no packaged equivalent —\n"
                     "  use the open-source alternative that driverman recommends,\n"
                     "  or install the vendor's official .run file manually.\n"
                  << C_RESET;
        return false;
    }
    ...
    // Run driverman itself as a normal user (not via sudo) for this to work;
    // runInstall() above already adds its own sudo prefix as needed.
    if (geteuid() == 0) {
        std::cerr << C_RED
                  << "[driverman] Can't build AUR packages while running as root.\n"
                     "  Run driverman as a regular user with sudo rights instead of\n"
                     "  via `sudo driverman ...` — it escalates only the package\n"
                     "  calls it actually needs, and AUR builds require a non-root user.\n"
                  << C_RESET;
        return false;
    }
    ...
    if (helper.empty()) {
        std::cerr << C_RED
                  << "[driverman] No AUR helper found (paru/yay/trizen).\n"
                     "  Install paru: https://aur.archlinux.org/packages/paru\n"
                     "  Then re-run: driverman --install\n"
                  << C_RESET;
        return false;
    }
    ...
    std::cout << C_CYAN << "[driverman] Running (AUR): " << cmd << C_RESET << "\n";
    return system(cmd.c_str()) == 0;
}
...
std::vector<DriverInfo> DriverManager::buildAmdList(const GPUInfo& gpu) {
    ...
    auto add = [&](const std::string& pkg, const std::string& desc,
        ...
        DriverInfo d;
        std::string mapped = platformPkgName(pkg);
        d.packageName = mapped.empty() ? pkg : mapped;
        d.description = desc;
        d.type        = type;
...
std::vector<DriverInfo> DriverManager::buildViaList(const GPUInfo& gpu) {
    ...
    std::cout << C_YELLOW
              << "  [!] VIA integrated GPUs share the OpenChrome driver with S3 Chrome\n"
                 ...
                 "      On Arch xf86-video-openchrome is AUR-only — needs paru/yay.\n"
              << C_RESET;
...
NvidiaGen DriverManager::detectNvidiaGen(const GPUInfo& gpu) {
    ...
    for (const char* model : ambiguousRebadges) {
        if (strContains(n, model)) {
            std::cerr << "\033[33m[driverman] \"" << n << "\" was sold as both Fermi "
...
std::vector<DriverInfo> DriverManager::buildNvidiaList(const GPUInfo& gpu) {
    ...
    // The legacy AUR driver branches (580xx/470xx/390xx/340xx) only exist on
    // Arch (via an AUR helper). On Debian they map to no package at all, so
    // the fallback becomes the open-source (nouveau) stack.
    bool legacyOk = (detectPlatform() == Platform::ARCH);
    ...
    case NvidiaGen::TURING: {
        ...
        // Some Arch-based distros (e.g. pearOS on its CachyOS kernel) publish a
        // precompiled "<kernel-package>-nvidia-open" module built for that
        // EXACT kernel — no DKMS build, no kernel headers, no compile time.
        // Prefer it over the generic DKMS package whenever it exists. (Debian
        // has no per-kernel prebuilt modules, so this is Arch-only.)
        std::string prebuiltOpen;
        if (detectPlatform() == Platform::ARCH) {
            std::string kernelPkg = detectRunningKernelPackage();
            prebuiltOpen = kernelPkg.empty() ? "" : (kernelPkg + "-nvidia-open");
        }
        ...
        if (hasPrebuiltOpen)
            add(prebuiltOpen, "NVIDIA open kernel module — precompiled for the running kernel "
                               "(" + detectRunningKernelPackage() + "), no DKMS build/headers needed",
    ...
    // (Arch/AUR only; on Debian driverman recommends the open-source stack).
    case NvidiaGen::VOLTA:
    case NvidiaGen::PASCAL:
    case NvidiaGen::MAXWELL: {
        bool propRec = !explicitOpen && legacyOk;
        bool nv2Rec  =  explicitOpen || !legacyOk;   // nouveau is the only always-available option
    ...
    // --prefer proprietary pulls in the AUR legacy DKMS package (Arch only).
    case NvidiaGen::KEPLER: {
        bool propRec = explicitProprietary && legacyOk;
        bool nv2Rec  = !propRec;
...
std::string DriverManager::detectKernelHeadersPackage() {
    std::string kernelPkg = detectRunningKernelPackage();
    if (kernelPkg.empty()) return "";
    if (detectPlatform() == Platform::DEBIAN) {
        // linux-image-6.1.0-18-amd64 -> linux-headers-6.1.0-18-amd64
        const std::string prefix = "linux-image-";
        if (kernelPkg.rfind(prefix, 0) == 0)
            return "linux-headers-" + kernelPkg.substr(prefix.size());
        return "";
    }
    return kernelPkg + "-headers";
}
...
static std::string queryPackageVersion(const std::string& pkg) {
    ...
    if (detectPlatform() == Platform::DEBIAN) {
        // Installed version via dpkg, repo candidate via apt-cache policy.
        std::string out = execCmd("dpkg-query -W -f='${Version}' " + name + " 2>/dev/null");
        std::string v = trimStr(out);
        if (!v.empty()) return v;
        std::string pol = execCmd("apt-cache policy " + name + " 2>/dev/null");
        std::istringstream ss(pol);
        std::string line;
        while (std::getline(ss, line)) {
            if (line.rfind("  Candidate:", 0) == 0) {
                v = trimStr(line.substr(std::string("  Candidate:").size()));
                if (v.find("(none)") == std::string::npos) return v;
                return "";
            }
        }
        return "";
    ...
    std::string out = execCmd("pacman -Qi " + name + " 2>/dev/null");
    if (out.empty()) out = execCmd("pacman -Si " + name + " 2>/dev/null");
    std::istringstream ss(out);
    std::string line;
    while (std::getline(ss, line)) {
...
std::vector<DriverInfo> DriverManager::buildList(const GPUInfo& gpu) {
    ...
    if (!list.empty()) {
        ...
        if (needsDkms) {
            ...
            if (!headersPkg.empty()) {
                ...
                h.fromAUR     = false;
                h.openSource  = true;
                h.available   = !platformPkgName(headersPkg).empty();
                ...
                if (!kernelVer.empty() && !headersVer.empty() && kernelVer != headersVer) {
                    ...
                    std::cerr << C_RED << C_BOLD
                              ...
                              << "      These must be the exact same version. Upgrade both together\n"
                              << C_RESET;
                }
                ...
                std::cerr << C_YELLOW
                          << "  [!] Could not auto-detect the kernel headers package for "
                             "the running kernel — install it manually before the DKMS "
                             "build runs.\n"
                          << C_RESET;
            }
        }
    ...
    // Asahi is an Arch-only distribution: on Debian there is no packaged
    // equivalent, so driverman points the user at the Asahi Linux project.
    ...
    for (const auto& d : drivers) {
        if (isIncompatibleInstalled(d)) {
            std::cout << C_RED << C_BOLD
                      ...
                      << "\n      Remove it: " << removeHintCmd() << " " << d.packageName
                      << C_RESET << "\n";
        }
    }
}
...
void DriverManager::listDriversJson() {
    ...
    std::cout << "{\"platform\":\"" << (detectPlatform() == Platform::DEBIAN ? "debian" : detectPlatform() == Platform::ARCH ? "arch" : "unknown")
              << "\",\"vmName\":\"" << jsonEscape(sysInfo_.vmName) << "\",\"gpus\":[";
    bool firstGpu = true;
    for (const auto& gpu : sysInfo_.gpus) {
        ...
        for (const auto& d : drivers) {
            ...
            std::cout << "{"
                       ...
                       << "\"available\":"     << (d.available   ? "true" : "false") << ","
...
void DriverManager::listDrivers() {
    if (sysInfo_.gpus.empty()) {
        std::cout << C_RED << "[driverman] No GPUs detected.\n" << C_RESET;
        return;
    }
    if (!sysInfo_.vmName.empty())
        std::cout << C_YELLOW << "[driverman] Running inside "
                  << sysInfo_.vmName << " VM\n" << C_RESET;
    ...
    for (const auto& gpu : sysInfo_.gpus) {
        ...
        for (const auto& orphan : findOrphanedDrivers(gpu, drivers, presentVendors)) {
            std::cout << C_RED << C_BOLD
                      ...
                      << "\n      Remove it: " << removeHintCmd() << " " << orphan.first
                      << C_RESET << "\n";
        }
    }
}
...
void DriverManager::regenerateInitramfs() {
    std::string cmd;
    if (detectPlatform() == Platform::DEBIAN) {
        if (execCmd("which update-initramfs 2>/dev/null").empty()) return;
        cmd = sudoCmd("update-initramfs -u -k all");
    } else {
        if (execCmd("which mkinitcpio 2>/dev/null").empty()) return;
        cmd = sudoCmd("mkinitcpio -P");
    }
    std::cout << C_CYAN << "[driverman] Running: " << cmd << "\n" << C_RESET;
    if (system(cmd.c_str()) != 0)
        std::cerr << C_RED << "  [!] initramfs rebuild failed — check the output above.\n" << C_RESET;
}
...
void DriverManager::applyKernelParams(const std::string& params) {
        ...
        if (pos != std::string::npos) {
            ...
            if (valEnd != std::string::npos) {
                ...
                if (!allTokensPresent(existing, params)) {
                    std::cout << C_CYAN
                              << "[driverman] Adding kernel params to GRUB_CMDLINE_LINUX_DEFAULT: "
                              << params << "\n" << C_RESET;
                    system(sudoCmd("cp /etc/default/grub /etc/default/grub.driverman.bak").c_str());
    ...
    while (std::getline(entryList, path)) {
        ...
        std::cout << C_CYAN << "[driverman] Adding kernel params to " << path << ": "
                  << params << "\n" << C_RESET;
        system(sudoCmd("cp " + path + " " + path + ".driverman.bak").c_str());
        if (writeFileAsRoot(path, newContent))
            appliedAny = true;
        else
            std::cerr << C_RED << "  [!] Failed to write " << path
                                 << " (backup at " << path << ".driverman.bak).\n" << C_RESET;
    }
...
bool DriverManager::cleanAllDrivers(bool noConfirm) {
    ...
    if (toRemove.empty()) {
        std::cout << C_GREEN
                  << "[driverman] No known GPU driver packages installed — nothing to clean.\n"
                  << C_RESET;
        return true;
    }

    std::cout << C_YELLOW << C_BOLD
              << "[driverman] --clean will remove ALL of the following GPU driver "
    ...
    if (!noConfirm_) {
        ...
        if (answer != "y" && answer != "Y") {
            std::cout << "[driverman] Aborted — nothing removed.\n";
            return false;
        }
    }

    bool ok = runRemove(toRemove);
    if (ok)
        std::cout << C_GREEN << C_BOLD
                  << "[driverman] Clean complete. Run --install to set up the correct "
...
bool DriverManager::installBestDriver(bool noConfirm) {
    ...
    if (sysInfo_.gpus.empty()) {
        std::cerr << C_RED << "[driverman] No GPUs detected.\n" << C_RESET;
        return false;
    }
    ...
    for (const auto& gpu : sysInfo_.gpus) {
        ...
        for (const auto& d : drivers) {
            ...
            // A package that has no installable equivalent on this platform
            // (e.g. AUR-only legacy NVIDIA branches on Debian) is skipped.
            if (!d.available) {
                std::cout << C_YELLOW
                          << "  ⏭ Skipping '" << d.packageName
                          << "' — not available on this platform.\n" << C_RESET;
                continue;
            }
            // On Debian, double-check the mapped name actually exists in the
            // configured apt repositories (some, like the NVIDIA open kernel
            // module or intel-media-driver, live in contrib/non-free).
            if (detectPlatform() == Platform::DEBIAN && !isAvailable(d.packageName)) {
                std::cout << C_YELLOW
                          << "  ⏭ Skipping '" << d.packageName
                          << "' — not found in the apt repositories.\n"
                             "      Enable the 'contrib' and 'non-free' components if it's an\n"
                             "      NVIDIA/Intel media driver package (run: sudo apt-get update).\n"
                          << C_RESET;
                continue;
            }
        ...
        if (nothingToInstall) {
            std::cout << C_GREEN << C_BOLD
                      << "  OK — all recommended drivers already installed (or no\n"
                         "  installable recommendation for this platform):\n"
                      << C_RESET;
            ...
            if (!pacmanPkgs.empty()) {
                std::cout << C_CYAN << "  Packages to install"
                          << (detectPlatform() == Platform::DEBIAN ? " (apt):" : " (pacman):")
                          << "\n" << C_RESET;
                for (const auto& p : pacmanPkgs) std::cout << "    + " << p << "\n";
                if (!runInstall(pacmanPkgs)) {
                    std::cerr << C_RED << "  [!] Package install failed.\n" << C_RESET;
                    std::cerr << C_YELLOW
                              << "      A package conflict may be blocking it. Fix it manually:\n"
                              << "      " << (detectPlatform() == Platform::DEBIAN
                                    ? "sudo apt-get install " : "sudo pacman -S --needed ")
                              << [&]{ std::string s; for (auto&p: pacmanPkgs) s += p + " "; return s; }()
                              << "\n" << C_RESET;
                    allOk = false;
        ...
        if (noConfirm_ && allOk) {
            ...
            if (!toRemove.empty()) {
                ...
                for (const auto& p : toRemove) std::cout << p << " ";
                std::cout << "\n";
                if (!runRemove(toRemove))
                    std::cerr << C_RED << "  [!] Failed to remove some driver(s) — "
                                         "remove manually: " << removeHintCmd() << " "

...
static void printBanner() {
    std::cout << C_CYAN C_BOLD
              << "  _                     _\n"
                 " | |   _ __   __ _  ___| |_ ___ _ __ _ __ ___ _ __   ___ _ __  _   _\n"
                 " | |  | '_ \\ / _` |/ __| __/ _ \\ '__| '_ ` _ \\ '_ \\ / _ \\ '_ \\| | | |\n"
                 " | |__| | | | (_| | (__| ||  __/ |  | | | | | | | | |  __/ | | | |_| |\n"
                 " |____|_| |_|\\__,_|\\___|\\__\\___|_|  |_| |_| |_|_| |_|\\___|_| |_|\\__, |\n"
                 "                                                                  |___/ \n"
              << C_RESET << "\n";
}
...
static void printUsage(const char* prog) {
    ...
    bool     arch   = (plat == Platform::ARCH);
    bool     debian = (plat == Platform::DEBIAN);

    std::string sudoNote = arch
        ? std::string("user (NOT via `sudo ") + prog + " ...`) — pacman calls "
          "self-escalate with sudo, while AUR builds need a non-root user."
        : std::string("user (NOT via `sudo ") + prog + " ...`) — apt calls "
          "self-escalate with sudo.";
    const char* noconfirmNote = arch
        ...
        : "      all apt prompts (DEBIAN_FRONTEND=noninteractive) and, for DKMS\n"
          "      packages, rebuilds the initramfs afterwards (update-initramfs -u)\n"
          "      — fully unattended, for install scripts.";
    ...
    std::cout
        ...
        << (debian ? "                           NVIDIA Maxwell/Pascal/Volta\n"
                     "                                             → nvidia-driver\n"
                   : "                           NVIDIA Maxwell/Pascal/Volta    → nvidia-580xx-dkms (AUR)\n")
        << (debian ? "                           NVIDIA Kepler/Fermi/8-9xxx → none (open-source only)\n"
        ...
        << C_DIM
        << "  (default, no flag)      NVIDIA Turing+                 → "
        << (debian ? "nvidia-open-kernel-dkms (open kernel)\n" : "nvidia-open-dkms (open kernel)\n")
        << "                           NVIDIA Maxwell/Pascal/Volta    → "
        << (debian ? "nvidia-driver\n" : "nvidia-580xx-dkms (AUR)\n")
        << (debian ? "                           NVIDIA Kepler/Fermi/8-9xxx     → xserver-xorg-video-nouveau + mesa\n"
...
static DriverPreference parsePreference(int argc, char* argv[], int start) {
    for (int i = start; i < argc; ++i) {
        ...
        if (arg == "--prefer" && i + 1 < argc) {
            ...
            std::cerr << C_YELLOW << "[driverman] Unknown --prefer value: " << val
...
static void printDetect(const SystemInfo& info) {
    if (!info.vmName.empty()) {
        std::cout << C_YELLOW C_BOLD
                  << "[driverman] Virtualized environment: " << info.vmName
                  << C_RESET << "\n";
    }

    if (info.gpus.empty()) {
        std::cout << C_RED << "[driverman] No GPU detected.\n" << C_RESET;
        return;
    }

    std::cout << C_BOLD << "[driverman] Detected GPU(s):\n" << C_RESET;
    int idx = 1;
...
int main(int argc, char* argv[]) {
    ...
    if (flag == "--simulate") {
        if (argc < 3) {
            std::cerr << C_RED << "[driverman] Usage: " << argv[0]
        ...
        if (g.vendor == GPUVendor::UNKNOWN)
            std::cerr << C_YELLOW
                      << "[driverman] Could not guess a vendor from \"" << name
    ...
    if (flag == "--install") {
        // No root requirement here on purpose: package-manager calls
        // regenerateInitramfs() escalate with `sudo` themselves as needed,
        // while AUR packages (Arch) must be built as a regular user (AUR
        // helpers refuse to run as root). Run this as the normal user with
        // sudo rights, not via `sudo driverman ...`.
        if (geteuid() == 0 && detectPlatform() == Platform::ARCH)
            std::cerr << C_YELLOW
                      << "[driverman] Warning: running as root. Packages needing an AUR\n"
                         "  helper (legacy NVIDIA branches, some S3/VIA drivers) will fail —\n"
                         "  run driverman as a regular (sudo-capable) user instead.\n"
                      << C_RESET;
        ...
        if (noConfirm)
            std::cout << C_YELLOW
                      << "[driverman] --noconfirm: installing unattended, no prompts.\n"
                      << C_RESET;
    ...
    std::cerr << C_RED << "[driverman] Unknown option: " << flag << C_RESET << "\n\n";
    printUsage(argv[0]);
    return 1;
}
